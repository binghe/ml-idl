...
<INITIAL>{ws}+ => (continue());
<INITIAL>\"([^"]|\\\")*\" => (Tokens.STRING (case (fn x => SOME (x)) (yytext) of NONE => error ("cannot convert string",!pos,!pos) | SOME (v) => v,!pos,!pos));
<INITIAL>(~)?[0-9]+ => (Tokens.INT32 (case (Int32.fromString) (yytext) of NONE => error ("cannot convert int32",!pos,!pos) | SOME (v) => v,!pos,!pos));
<INITIAL>"0wx"[0-9a-fA-F]+ => (Tokens.WORD32 (case (Word32.fromString) (yytext) of NONE => error ("cannot convert word32",!pos,!pos) | SOME (v) => v,!pos,!pos));
<INITIAL>[0-9a-fA-F]+"-"[0-9a-fA-F]+"-"[0-9a-fA-F]+"-"[0-9a-fA-F]+"-"[0-9a-fA-F]+ => (Tokens.UUID (case (fn x => SOME (x)) (yytext) of NONE => error ("cannot convert string",!pos,!pos) | SOME (v) => v,!pos,!pos));
<INITIAL>".." => (Tokens.DDOT (!pos,!pos));
...
<INITIAL>"*" => (Tokens.ASTERISK (case (fn x => SOME (x)) (yytext) of NONE => error ("cannot convert string",!pos,!pos) | SOME (v) => v,!pos,!pos));
<INITIAL>"'" => (Tokens.QUOTE (!pos,!pos));
<INITIAL>"=" => (Tokens.EQSIGN (!pos,!pos));
<INITIAL>"-" => (Tokens.DASH (!pos,!pos));
<INITIAL>{identifier} => (K.keyword (yytext,!pos,!pos));
<INITIAL>"//" => (YYBEGIN COMMENT0; continue ());
<COMMENT0>{eol} => (YYBEGIN INITIAL;pos := (!pos)+1; continue());
<COMMENT0>. => (continue ());
<INITIAL>"/*" => (YYBEGIN COMMENT1; comment1 := 1; continue ());
<COMMENT1>"/*" => (comment1 := (!comment1)+1; continue ());
<COMMENT1>{eol} => (pos := (!pos)+1; continue ());
<COMMENT1>"*/" => (comment1 := (!comment1)-1; if (!comment1)=0 then (YYBEGIN INITIAL; continue ())else continue ());
<COMMENT1>. => (continue ());
